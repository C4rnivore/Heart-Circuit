...
from neir import mUNet
from lv import area2cont, get_contour_points, interpolate_contour
...
for img in tqdm(imgs, total=len(imgs)):
    out = model(torch.tensor(resize(img, (512, 512), preserve_range=True, order=1)).unsqueeze(0).unsqueeze(0).float())
